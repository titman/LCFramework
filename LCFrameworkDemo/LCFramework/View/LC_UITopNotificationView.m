...
-(void) initSelf
{
    ...
    if ([parentView isKindOfClass:[UIScrollView class]]) {
        [parentView addObserver:self forKeyPath:@"contentOffset" options:NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld context:NULL];
    }
}
...
-(void) layout
{
    if (self.imageView.image) {
        ...
        self.label.frame = LC_RECT_CREATE(labelX, distanceSide, parentView.viewFrameWidth - labelX - distanceSide, LCUITopNotificationViewHeight - distanceSide * 2);

        
    }else{
        
        float distanceSide = (LCUITopNotificationViewHeight - 4);
        self.label.frame = CGRectMake(2, 2, parentView.viewFrameWidth, distanceSide);
    }
}
...
{
    if ([object isEqual:parentView]) {
        if ([keyPath isEqualToString:@"contentOffset"]) {
            self.frame = [self getFrameFromView:parentView];
        }
    }
}
