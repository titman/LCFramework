...
-(void) viewDidAppear:(BOOL)animated
{
    ...
    alertView.clickBlock = ^(BOOL cancel, NSInteger index){
...
-(LC_UITableViewCell *) tableView:(LC_UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary * crashData = LC_DS(___ds1)[indexPath.row];
    
    [cell setTitle:crashData[@"date"]
          subTitle:crashData[@"exception"]];
...
-(void) tableView:(LC_UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(crashData[@"callStackSymbols"]);
}
