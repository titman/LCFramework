...
@interface LCAlertViewStack : NSObject

@property (nonatomic,retain) NSMutableArray *alertViews;

+ (LCAlertViewStack *)sharedInstance;

- (void)push:(LCAlertView *)alertView;
- (void)pop:(LCAlertView *)alertView;
         ...
         completion:(LCAlertViewCompletionBlock)completion
{
    ...
    if (self) {
        ...
        _alertWindow = [[self windowWithLevel:UIWindowLevelAlert] retain];
        ...
        _backgroundView = [[[UIView alloc] initWithFrame:frame] autorelease];
        _backgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.25];
        _backgroundView.alpha = 0;
        [self.view addSubview:_backgroundView];
        
        _alertView = [[[UIView alloc] init] autorelease];
        ...
        _titleLabel.textColor = LC_RGB(56, 56, 56);
        ...
        _messageLabel.textColor = LC_RGB(56, 56, 56);
...
- (UIButton *)genericButton
{
    ...
    [button setTitleColor:LC_RGB(56, 56, 56) forState:UIControlStateNormal];
    [button setTitleColor:LC_RGB(86, 86, 86) forState:UIControlStateHighlighted];
...
- (void)setupGestures
{
    self.tap = [[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(dismiss:)] autorelease];
...
- (void)show
{
    [[LCAlertViewStack sharedInstance] push:self];
}
...
- (void)dismiss:(id)sender animated:(BOOL)animated
{
    ...
    if ([[[LCAlertViewStack sharedInstance] alertViews] count] == 1) {
        if (animated) {
    ...
    [UIView animateWithDuration:(animated ? 0.2 : 0) animations:^{
        ...
        [[LCAlertViewStack sharedInstance] pop:self];
        [self.view removeFromSuperview];
    }];
    ...
    if (self.clickBlock) {
        ...
        self.clickBlock(cancelled, buttonIndex);
    }
}
...
//    self.alertView.alpha = 0;
//    self.alertView.transform = CGAffineTransformConcat(self.alertView.transform, CGAffineTransformMakeScale(0.5f, 0.5f));
//
//    
    /* POP */
...
//    
//    [self.alertView pop_addAnimation:springAnimation forKey:@"changeposition"];
//    
...
//    
//    [self.alertView pop_addAnimation:alphaAnimation forKey:@"changealpha"];
}
...
//    POPSpringAnimation * springAnimation = [POPSpringAnimation animationWithPropertyNamed:kPOPViewScaleXY];
//    
//    springAnimation.toValue = [NSValue valueWithCGSize:CGSizeMake(0.5 , 0.5)];
//    springAnimation.springBounciness = 10.0;
//    springAnimation.springSpeed = 10.0;
//    springAnimation.removedOnCompletion = YES;
//    
...
+ (instancetype)showAlertWithTitle:(NSString *)title
                       ...
                       cancelTitle:(NSString *)cancelTitle
                        completion:(LCAlertViewCompletionBlock)completion
{
    LCAlertView *alertView = [[self alloc] initWithTitle:title
                                                 message:message
                                             cancelTitle:cancelTitle
                                              otherTitle:nil
                                             contentView:nil
...
@implementation LCAlertViewStack
...
{
    static LCAlertViewStack *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[LCAlertViewStack alloc] init];
...
- (void)push:(LCAlertView *)alertView
{
    ...
    for (LCAlertView *av in self.alertViews) {
        if (av != alertView) {
            [av hide];
        }
    }
...
- (void)pop:(LCAlertView *)alertView
{
    ...
    LCAlertView *last = [self.alertViews lastObject];
    if (last) {
        [last showInternal];
    }
}
