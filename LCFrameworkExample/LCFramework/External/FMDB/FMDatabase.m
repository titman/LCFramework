    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                ...
                ERROR(@"Database busy, unable to close");
                return NO;
            }
            ...
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, 0x00)) !=0) {
                    ERROR(@"Closing leaked statement");
                    sqlite3_finalize(pStmt);
                }
            }
        }
        else if (SQLITE_OK != rc) {
            ERROR(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (void)setCachedStatement:(FMStatement*)statement forQuery:(NSString*)query {
    ...
    query = [[query copy] autorelease]; // in case we got handed in a mutable string...
...
- (void)warnInUse {
    ERROR(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        ERROR(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        INFO(@"[FMDatabase] %@ executeQuery: %@", self, sql);
    }
    ...
    if (!pStmt) {
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                if (_busyRetryTimeout && (numberOfRetries++ > _busyRetryTimeout)) {
                    ERROR(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                    ERROR(@"Database busy");
                    sqlite3_finalize(pStmt);
                    _isExecutingStatement = NO;
                    return nil;
                ...
                if (_logsErrors) {
                    ERROR(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                    ERROR(@"DB Query: %@", sql);
                    ERROR(@"DB Path: %@", _databasePath);
#ifndef NS_BLOCK_ASSERTIONS
                    if (_crashOnErrors) {
                        abort();
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (namedIdx > 0) {
                ...
                ERROR(@"Could not find index for %@", dictionaryKey);
            }
        }
    ...
    if (idx != queryCount) {
        ERROR(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_traceExecution && sql) {
        INFO(@"[FMDatabase] %@ executeUpdate: %@", self, sql);
    }
    ...
    if (idx != queryCount) {
        ERROR(@"Error: the bind count is not correct for the # of variables (%@) (executeUpdate)", sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    ERROR(@"Unexpected result from sqlite3_reset (%d) eu", rc);
                }
            }
            usleep(20);
            ...
            ERROR(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
            ERROR(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            ERROR(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
            ERROR(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            ERROR(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
            ERROR(@"DB Query: %@", sql);
        }
    ...
    if (closeErrorCode != SQLITE_OK) {
        ERROR(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
        ERROR(@"DB Query: %@", sql);
    }
...
- (BOOL)executeUpdate:(NSString*)sql, ... {
    ...
    if (!result) {
        ...
        ERROR(@"SQLite execute update fail : %@",self.lastErrorMessage);
    }
