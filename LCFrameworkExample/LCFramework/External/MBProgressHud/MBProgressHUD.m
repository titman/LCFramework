...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.minSize = CGSizeMake(120, 100);
		self.square = NO;
        self.hudRadius = 10;
...
//                
//                [self pop_addAnimation:springAnimation forKey:@"changeposition"];
//                
...
//                
//                [self pop_addAnimation:alphaAnimation forKey:@"changealpha"];
//                
//            }
        }
        else{
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
        ...
        /* For NextApp */
        //self.indicator = [NASpinView whiteActivityIndicatorView];
...
- (void)layoutSubviews {
	...
	CGFloat maxWidth = (bounds.size.width - 4 * margin) > (LC_DEVICE_WIDTH - 4 * margin) ? (LC_DEVICE_WIDTH - 4 * margin) : (bounds.size.width - 4 * margin) ;
