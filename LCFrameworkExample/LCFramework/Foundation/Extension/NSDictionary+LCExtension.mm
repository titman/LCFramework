...
- (id)allKeysSelectKeyAtIndex:(NSInteger)index;
{
    if (index + 1 > self.allKeys.count) {
        ERROR(@"NSDictionary+LCExtension - allKeysSelectKeyAtIndex 越界 keys count : %d index : %d",self.allKeys.count,index);
        return nil;
    }
...
//    }
	...
	for ( Class clazzType = clazz; clazzType != [NSObject class]; )
	{
		...
		for ( NSUInteger i = 0; i < propertyCount; i++ )
		{
			...
			NSUInteger		type = [LC_TypeEncoding typeOf:attr];
			...
			if ( tempValue )
			{
				...
				{
					NSString * className = [LC_TypeEncoding classNameOfAttribute:attr];
...
@implementation NSMutableDictionary(LCExtension)
