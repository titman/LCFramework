...
{
    [[LC_AppVersion LCInstance] checkForNewVersion];
}
...
- (void) handleRequest:(LC_HTTPRequest *)request
{
    if (request.succeed) {
        ...
        if ([[resultData objectForKey:@"resultCount"] intValue] == 0) {
            ...
            INFO(@"[LC_AppVersion] App version check failed : can't find the \"%@\" application in AppStore.",[LC_SystemInfo appIdentifier]);
            ...
            if (newVersionNumber) {
                INFO(@"[LC_AppVersion] Check finished : new %@",newVersionNumber);
                ...
                INFO(@"[LC_AppVersion] Check finished : no new version.");
//                self.hasNewVersion = NO;
//                self.theNewVersionDetails = nil;
//                self.theNewVersionNumber = nil;
//                self.theNewVersionURL = nil;
    ...
    if (self.alertStyle == LC_APPVERSION_ALERT_STYLE_ALERT) {
        ...
        alertView.clickBlock = ^(BOOL cancel, NSInteger clickIndex){
            ...
            if (clickIndex != 0) {
                ...
                [[UIApplication sharedApplication] openURL:[NSURL URLWithString:nRetainSelf.theNewVersionURL]];
            }
